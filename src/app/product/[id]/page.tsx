...
import { ShoppingCart, Heart, Calendar, Clock, Info, MapPin, Truck, Store } from "lucide-react";
import { motion } from "framer-motion";
import { WEIGHT_OPTIONS, DELIVERY_SLOTS, EGG_TYPES, REGULAR_CAKES, GELATOS, TEA_TIME_CAKES } from "@/constants";
...
const ALL_PRODUCTS_LIST = [
  ...
  ...TEA_TIME_CAKES.map((c, index) => ({
    ...c,
    description: "Perfect companion for your tea or coffee. " + c.name + " made fresh every day.",
    images: [c.image, "https://images.unsplash.com/photo-1515037893149-de7f840978e2?q=80&w=600"],
    rating: 4.7,
    reviews: 80 + index
  })),
  {
  ...
  ...GELATOS.map((g, index) => ({
    ...g,
    description: "Authentic Italian-style gelato made with the finest ingredients.",
    images: [g.image],
    rating: 5.0,
    reviews: 45 + index
  }))
...
export default function ProductDetailPage() {
  ...
  const currentPrice = product.price * getWeightMultiplier(selectedWeight);

  const handleAddToCart = () => {
    if (!deliveryDate) {
      toast.error(`Please select a ${deliveryMethod === "Pickup" ? "pickup" : "delivery"} date`);
      return;
    }

    if (deliveryMethod === "Delivery" && !deliverySlot) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div className="space-y-8">
            <div>
              ...
              <p className="text-3xl font-bold text-red-primary mt-6">₹{formatPrice(currentPrice)}</p>
            </div>
            ...
            <div className="space-y-6 border-t border-dark/5 pt-8">
              ...
              {/* Delivery Method */}
              <div>
                <label className="block text-sm font-bold text-dark uppercase tracking-widest mb-4">Delivery Method</label>
                <div className="flex gap-4">
                  <button
                    onClick={() => setDeliveryMethod("Pickup")}
                    className={cn(
                      "flex-1 py-3 rounded-2xl border-2 flex items-center justify-center gap-3 transition-all font-medium",
                      deliveryMethod === "Pickup" ? "border-dark bg-dark text-cream" : "border-dark/10 text-dark/60 hover:border-dark/20"
                    )}
                  >
                  ...
                  <button
                    ...
                    className={cn(
                      "flex-1 py-3 rounded-2xl border-2 flex items-center justify-center gap-3 transition-all font-medium",
                      deliveryMethod === "Delivery" ? "border-dark bg-dark text-cream" : "border-dark/10 text-dark/60 hover:border-dark/20"
                    )}
                  >
